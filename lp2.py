...
## process directory => list of processable fileobjects
## process data => que of docs for preprocessing
## preprocess for es => que actions for indexing / updates
## put data in es

#######notes
#for file status in mem shared dict + thread that syncs to disk
...
def create_doc(file,encoding=None):
...
def process_file(input_file,encoding=None):
...
def main():
    ...
    parser.add_argument("-l",help="logfile for status data for resuming later",type=str,default="lp_process.log")
    ...
    for x in get_files(args.folder):
        for y in create_doc(x):
            print(y)
